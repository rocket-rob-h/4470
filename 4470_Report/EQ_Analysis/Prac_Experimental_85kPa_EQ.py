...
print(f"Me from A_Astar at Nozzle exit EQ is {Me:.3f}")

T5_Te = stagnation_temperature_ratio(gamma_5, Me)
print(f"T5_Te ratio from Me is {T5_Te:.3f} ")

Te = Bottom_ratio(T_5,T5_Te)
print(f"Te Exit Temp at EQ is {Te:.3f} K")

P5_Pe = stagnation_pressure_ratio(gamma_5, Me)
print(f"P5_Pe: P5 on Pe ratio at EQ is {P5_Pe:.3f}")

Pe = Bottom_ratio(P_5, P5_Pe)
print(f"Exit Pressure Pe is {Pe:.3f} Pa")

rho5_rho_e = shock_density_ratio(Me, gamma_5)
print(f"rho5_rho_e: Density ratio p5/pe at EQ is {rho5_rho_e:.3f}")

rho_e = Bottom_ratio(rho_5,rho5_rho_e)
print(f"rho_e: Density at Exit EQ is {rho_e:.7f}")


print()
print( " - Plate Properties from EQ Forced State 5 Values - "*2)
...
P5_P2 = P_5 / P_2
print(f"Experimental P5 from graph and P2 from Ms ratio is {P5_P2:2f}")
...
Tailoring85kpa = EQN(U_2, a_3, gamma_4, P5_P2)

print(f"EQN of  system is {Tailoring85kpa:.2f}")

print()
print("Finished Second Analysis")
